// server/tests/precificacaoServiceContaScoped.test.js
//
// Mesma classe de bug do modeloBaseCustos/planilhaPrecificacaoSemBase:
// gerarPreviewPrecificacaoMl monta o path do ML com o mlUserId da conta
// selecionada (`/users/${mlUserId}/items/search`) mas chamava `mlFetch` sem
// `{ mlUserId }` — a escolha do TOKEN caía no fallback/principal do
// cliente. Cliente com 2 contas ML e a Conta 1 como principal: pedir a
// Conta 2 usaria o seller 222 no path só que autenticado com o token da
// Conta 1 (111), e o ML responde 403 "Searching another user items is
// restricted."
//
...
class MemoryDb {
  ...
  async query(sql, params = []) {
    ...
    // base_cliente_vinculos por conta (obterBaseDaConta): nenhum vínculo
    // direto por conta neste fixture — cai no fallback "legado" por cliente.
    if (q.includes("FROM base_cliente_vinculos v") && q.includes("v.cliente_conta_id = $1")) {
      return { rows: [] };
    }
    // base_cliente_vinculos por cliente (buscarBasesMeliDoCliente e o
    // fallback "legado" de obterBaseDaConta) — mesma base vinculada nos dois.
...
async function run() {
  ...
  try {
    ...
    await gerarPreviewPrecificacaoMl({ clienteSlugRaw: "cliente-x", clienteContaId: 102 });
    ...
    chamadasFetch.length = 0;
    await gerarPreviewPrecificacaoMl({ clienteSlugRaw: "cliente-x", clienteContaId: 101 });
    ...
    console.log(`\n✓ precificacaoServiceContaScoped: ${checks} verificações`);
  } finally {
